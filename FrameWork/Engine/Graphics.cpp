...
Graphics::Graphics( HWNDKey& key )
{
	...
	pSysBuffer = reinterpret_cast<Colour*>( 
		_aligned_malloc( sizeof( Colour ) * Graphics::ScreenWidth * Graphics::ScreenHeight,16u ) );
}
...
void Graphics::EndFrame()
{
	...
	const size_t dstPitch = mappedSysBufferTexture.RowPitch / sizeof( Colour );
	const size_t srcPitch = Graphics::ScreenWidth;
	const size_t rowBytes = srcPitch * sizeof( Colour );
...
{
	// clear the sysbuffer
	memset( pSysBuffer,0u,sizeof( Colour ) * Graphics::ScreenHeight * Graphics::ScreenWidth );
}

void Graphics::PutPixel( int x,int y,Colour c )
{
	assert( x >= 0 );
...
void Graphics::DrawRect(int x0, int y0, int x1, int y1, Colour c)
{
	if (x0 > x1)
	{
		std::swap(x0, x1);
...
void Graphics::DrawCircle(int x, int y, int radius, Colour c)
{
